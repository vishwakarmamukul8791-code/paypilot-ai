...
from app.services.payment_service import begin_serialized_payment_transaction, payment_execution_lock
...
@router.post("/accounts/{account_id}/funds", response_model=DashboardOut)
def add_simulated_funds(account_id: int, payload: FundsAdd, x_demo_session: str | None = Header(default=None, alias="X-Demo-Session"), db: Session = Depends(get_db)):
    sid = _session(db, x_demo_session)
    with payment_execution_lock(sid):
        begin_serialized_payment_transaction(db, sid, account_id)
        account = _account_or_409(db, sid, account_id)
        if account.balance + payload.amount > 10000000:
            raise HTTPException(status_code=422, detail="Simulation account balance cannot exceed ₹1,00,00,000.")
        account.balance += payload.amount
        txn = Transaction(
            session_id=sid,
            account_id=account.id,
            txn_id=f"PPA-{datetime.now(timezone.utc):%Y%m%d}-{uuid.uuid4().hex[:8].upper()}",
            beneficiary_name="Simulated funds added",
            amount=payload.amount,
            direction="CREDIT",
            category="add_money",
            status="COMPLETED",
            risk_score=0,
            idempotency_key=f"funds-{uuid.uuid4()}",
        )
        db.add(txn)
        db.commit()
    return _dashboard_payload(db, sid)
