...
export default function Setup({ data, busy, error, actions, onTryPrompt }) {
  ...
  const targets = data?.beneficiaries || []
  ...
  const submitFunds = async (e) => {
    ...
    const ok = await actions.addFunds(fundingId, Number(fundAmount))
    if (ok) { setFundingId(null); setFundAmount('') }
  }
    ...
    <div className="page-stack manage-page">
      ...
      {section === 'accounts' && <div className="manage-grid">
        <section className="panel form-panel">
          ...
          <div className="security-note">Balances cannot be silently edited. Use <b>Add money</b> so every simulated balance change creates a ledger entry.</div>
        </section>
        <section className="panel">
          ...
          <div className="account-manage-list">{accounts.length === 0 ? <div className="empty-state">Create your first account to unlock payments.</div> : accounts.map((item) => <div className={`manage-account-card ${item.is_primary ? 'primary-account' : ''}`} key={item.id}><div className="manage-account-top"><div><strong>{item.nickname}</strong><small>{item.account_type} · {item.masked_account}</small></div><span className={`account-state ${item.is_active ? '' : 'paused'}`}>{item.is_primary ? 'PRIMARY' : item.is_active ? 'ACTIVE' : 'PAUSED'}</span></div><div className="manage-balance">{money(item.balance)}</div><div className="manage-account-meta"><span>Daily limit {money(item.daily_limit)}</span><span>{item.owner_name}</span></div><div className="row-actions"><button className="secondary" onClick={() => editAccount(item)}>Edit</button>{!item.is_primary && item.is_active && <button className="secondary" disabled={busy} onClick={() => actions.setPrimary(item.id)}>Make primary</button>}<button className="secondary" onClick={() => { setFundingId(item.id); setFundAmount('') }}>Add money</button></div>{fundingId === item.id && <form className="inline-fund" onSubmit={submitFunds}><input autoFocus required min="0.01" step="0.01" type="number" value={fundAmount} onChange={(e) => setFundAmount(e.target.value)} placeholder="Amount to add" /><button className="primary" disabled={busy}>Credit account</button><button type="button" className="text-button" onClick={() => setFundingId(null)}>Cancel</button></form>}</div>)}</div>
        </section>
      </div>}
