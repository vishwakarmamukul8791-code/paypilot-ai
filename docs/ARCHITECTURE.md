...
Account metadata (nickname, type, daily limit, active/paused state) is editable. Balances are intentionally not directly editable: simulated deposits use an `add_money` credit transaction so the ledger remains explainable and auditable.
